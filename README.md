...
| `extract_behaviour`                 | `overwrite`, `rename`, or `skip` | `skip`    | Determines how [unar](https://theunarchiver.com/command-line) deals with existing files when extracting an archive. `overwrite` results in [unar](https://theunarchiver.com/command-line) overwriting existing files when extracting. `rename` results in [unar](https://theunarchiver.com/command-line) renaming the new files with the same name as existing files. `skip` results in [unar](https://theunarchiver.com/command-line) skipping files that have the same name as existing files. Use the `man unar` command for more information. |
...
| `wraparound_file_navigation`        | `true` or `false`                | `false`   | Wrap around from the bottom to the top or from the top to the bottom when using the `arrow` and `parent-arrow` command to navigate.                                                                                                                                                                                                                                                                                                                                                                                                               |
